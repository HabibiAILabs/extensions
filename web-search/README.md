...
snippets. SearXNG `unresponsive_engines` entries are forwarded as sanitized `provider_errors` with a
`provider_status`. An unavailable provider sets `retryable=false`; the model is instructed not to
repeat that search in the same reaction and to report the concrete suspension/CAPTCHA/timeout.
