...
Memory contributes up to 20 events from the current event's causal chain, ordered oldest first, plus up to 20 locally embedded semantic matches. Duplicate events keep their causal placement. Contributions share a deterministic 96 KiB budget; oversized events are omitted and `truncated` is reported. The extension formats the resulting context as JSON text, while core independently enforces per-hook and combined context limits.
